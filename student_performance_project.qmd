...
results["Lasso"] = evaluate_and_report("Lasso (Test)", y_test, lasso_test_pred)
```
...
plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
plt.scatter(lasso_test_pred, residuals_lasso, alpha=0.5)
plt.axhline(0, color="red", linestyle="--")
plt.xlabel("Predicted")
plt.ylabel("Residual")
...
top_k = 15
top_idx = order[:top_k]
top_features = [(feature_names[i], lasso.coef_[i]) for i in top_idx]
df_imp = pd.DataFrame(top_features, columns=["Feature", "Coefficient"])

print(f"\nTop {top_k} features from Lasso:")
